      ...
      <Row :gutter="15">
        <Col span="4">
          ...
          <ul class="role-list-box">
            <li :class="{'active': r.selected}" v-for="r in stores.role.data" :key="r.name" @click="handleRoleClicked(r)">{{r.name}}</li>
          </ul>
        </Col>
        <Col span="20">
    ...
    checkNodePermission (checked, permissions, node) {
      ...
      if (checked) {
        for (let i = 0, len = permissions.length; i < len; i++) {
          const d = permissions[i]
          const index = this.selectedPermissions.indexOf(d.code)
          if (index === -1) {
